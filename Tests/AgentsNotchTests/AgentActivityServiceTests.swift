...
final class AgentActivityServiceTests: XCTestCase {
    ...
    @MainActor
    func testSilentRunningTurnSettlesWithoutAStopHook() {
        let service = AgentActivityService()
        let started = Date(timeIntervalSince1970: 100)
        service.ingest(AgentEvent(
            type: .activity,
            sessionId: "codex:silent",
            provider: .codex,
            activity: "Running",
            state: .running,
            timestamp: started
        ...
        XCTAssertFalse(service.settleSilentTurns(now: started.addingTimeInterval(30)))
        XCTAssertEqual(service.session(id: "codex:silent")?.state, .running)

        XCTAssertTrue(service.settleSilentTurns(now: started.addingTimeInterval(91)))
        let settled = service.session(id: "codex:silent")
        XCTAssertEqual(settled?.state, .completed)
        XCTAssertEqual(settled?.currentActivity, "Turn ended")
        XCTAssertFalse(settled?.isActive ?? true)
    }

    @MainActor
    func testSilentWatchDoesNotSettleAToolInFlightOrWaiter() {
        let service = AgentActivityService()
        let started = Date(timeIntervalSince1970: 100)
        service.ingest(AgentEvent(
            type: .toolStarted,
            sessionId: "claude-code:tool",
            provider: .claudeCode,
            activity: "Using tool",
            state: .executingTool,
            timestamp: started
        ...
        XCTAssertFalse(service.settleSilentTurns(now: started.addingTimeInterval(120)))
        XCTAssertEqual(service.session(id: "claude-code:tool")?.state, .executingTool)
        XCTAssertEqual(service.session(id: "cursor:wait")?.state, .waitingForUser)
    }

    @MainActor
    func testSilentParentWaitsForARecentlyActiveChild() {
        let service = AgentActivityService()
        let parentAt = Date(timeIntervalSince1970: 100)
        let childAt = parentAt.addingTimeInterval(80)
        service.ingest(AgentEvent(
            type: .activity,
        ...
        let now = parentAt.addingTimeInterval(91)
        XCTAssertFalse(service.settleSilentTurns(now: now))
        XCTAssertEqual(service.session(id: "grok:parent")?.state, .running)
        XCTAssertNil(service.nextSilentTurnDeadline(now: now))
    }
}
